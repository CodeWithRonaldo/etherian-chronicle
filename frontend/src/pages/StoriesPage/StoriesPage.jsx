...
const StoriesPage = () => {
  ...
  const filteredStories = useMemo(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (story) =>
          ...
          story?.chapters[0]?.chapertDetails?.tags?.some((tag) =>
    ...
    if (filters.genre !== "all") {
      filtered = filtered.filter(
        (story) => story.chapters[0]?.chapertDetails?.genre === filters.genre
      );
    }
    ...
    if (filters.status !== "all") {
      filtered = filtered.filter(
        (story) => convertStoryStatus(story.storyStatus) === filters.status
      );
    }
    ...
    switch (filters.sortBy) {
      ...
      //   break;
      case "votes":
        filtered.sort((a, b) => b.proposalYesVotes - a.proposalYesVotes);
        break;
      case "alphabetical":
